...
export const bookSlotHandler = async(req , res) => {
    ...
    const {sportName , credits , slotNumber , availableTickets , startTime , endTime , date , timeStamp} = req.body
    try {
        //before creating the booking need to check whether the time is 3 hours prior the slot or 20 min before the slot
        const bookingRequestTime = moment(timeStamp)
        ...
        const timeDifference = showStartTime.diff(bookingRequestTime, 'minutes')
        // console.log('timeDiff:', timeDifference)

        if(timeDifference <= 180 && timeDifference >= 20){
            //then book the slot
